...
export function usePushPythFeed() {
    ...
    return useMutation({
        mutationFn: async ({ feed }: { feed: PythFeed }): Promise<PushPythFeedResult> => {
            ...
            const feedAccount = feedPda(payer)
            ...
            if (existing) {
                ...
                if (!('pyth' in source)) {
                    throw new Error(
                        "This wallet's feed is a Manual feed — it can't accept Pyth updates.",
                    )
                }
                ...
                if (!bytesEqual(boundColl, collBytes) || !bytesEqual(boundLend, lendBytes)) {
                    throw new Error(
                        `This wallet's feed is already bound to a different pair. Feeds are immutable once created — use a different wallet to price ${feed.symbol}.`,
                    )
                }
            } else {
