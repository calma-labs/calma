...
fn test_flow() {
    ...
    svm.add_program(jbl_id, include_bytes!("../../../target/deploy/jbl.so")).unwrap();
    ...
    let (lend_vault, _) =
        Pubkey::find_program_address(&[b"lend_vault", pool_pk.as_ref()], &jbl_id);
    let (lp_mint, _) = Pubkey::find_program_address(&[b"lp_mint", pool_pk.as_ref()], &jbl_id);
    let (irm_config, _) =
        Pubkey::find_program_address(&[b"irm_config", pool_pk.as_ref()], &irm_id);
    ...
    send_ixs(
        ...
        &[Instruction::new_with_bytes(
            jbl_id,
            &jbl::instruction::DepositLent { amount: LEND_DEPOSIT }.data(),
    ...
    send_ixs(
        ...
        &[Instruction::new_with_bytes(
            jbl_id,
            &jbl::instruction::DepositCollateral { amount: COL_DEPOSIT }.data(),
    ...
    assert_eq!(read_token_balance(&svm, &lend_vault), LEND_DEPOSIT - BORROW_AMOUNT);
    ...
    send_ixs(
        ...
        &[Instruction::new_with_bytes(
            jbl_id,
            &jbl::instruction::WithdrawLent { shares: LEND_DEPOSIT }.data(),
