...
use crate::{
    amount_to_shares, amount_to_shares_burned, compute_interest, shares_to_amount, IrmRate,
    MathError, Market, Oracle, Position, PRICE_SCALE,
};
...
impl<M: Market> Core<M, (), (), (), NotAccrued> {
    pub fn new(market: M) -> Self {
        Core { market, irm: (), position: (), oracle: (), _state: PhantomData }
    }
}

impl<M: Market, I, P, O, S> Core<M, I, P, O, S> {
    pub fn with_irm<I2: IrmRate>(self, irm: I2) -> Core<M, I2, P, O, S> {
        Core { market: self.market, irm, position: self.position, oracle: self.oracle, _state: PhantomData }
    }
    pub fn with_position<P2: Position>(self, position: P2) -> Core<M, I, P2, O, S> {
        Core { market: self.market, irm: self.irm, position, oracle: self.oracle, _state: PhantomData }
    }
    pub fn with_oracle<O2: Oracle>(self, oracle: O2) -> Core<M, I, P, O2, S> {
        Core { market: self.market, irm: self.irm, position: self.position, oracle, _state: PhantomData }
    }
...
impl<M: Market, I: IrmRate, P, O> Core<M, I, P, O, NotAccrued> {
    pub fn accrue_interest(mut self) -> Option<Core<M, I, P, O, Accrued>> {
        let elapsed = (self.irm.current_ts().saturating_sub(self.market.last_update())).max(0) as u64;
        if elapsed > 0 {
        ...
        Some(Core { market: self.market, irm: self.irm, position: self.position, oracle: self.oracle, _state: PhantomData })
    }
}
...
impl<M: Market, I, P: Position, O: Oracle> Core<M, I, P, O, Accrued> {
    ...
    pub fn borrow_with_fee<E, F>(&mut self, amount: u64, total_debt_amount: u64, transfer: F) -> Result<u64, MathError<E>>
    where
        F: FnOnce(u64) -> Result<(), E>,
    {
        let oracle_price = self.oracle.price();
    ...
    pub fn withdraw_collateral<E, F>(&mut self, amount: u64, transfer: F) -> Result<u64, MathError<E>>
    where
        F: FnOnce(u64) -> Result<(), E>,
    {
        let oracle_price = self.oracle.price();
